...
// Standalone build: provide minimal rust:: namespace stubs instead of <rust/cxx.h>
namespace rust {
    template<typename T>
    struct Slice {
        const T *_data;
        ...
        Slice(const T *d, size_t l) : _data(d), _len(l) {}
...
// MSVC struct packing — all boot image headers must be tightly packed
#ifdef _MSC_VER
#pragma pack(push, 1)
#endif
