// Adapted from upstream: native/src/boot/compress.rs
// Changes: Replaced nix/OFlag file I/O with std::fs equivalents.
//          Removed C++ FFI functions (compress_bytes, decompress_bytes).
//          Compression/decompression logic is kept identical to upstream.
...
// Adapted: use std::fs instead of nix OFlag
...
pub(crate) fn decompress_cmd(infile: &Utf8CStr, outfile: Option<&Utf8CStr>) -> LoggedResult<()> {
    ...
    let input = if in_std {
        ...
        FileOrStd::File(File::open(infile.as_str())?)
    };
    ...
    let output = if let Some(outfile) = outfile {
        if outfile == "-" {
            ...
            FileOrStd::File(File::create(outfile.as_str())?)
        }
    } else if in_std {
        FileOrStd::StdOut
    } else {
    ...
    if rm_in {
        std::fs::remove_file(infile.as_str())?;
    }
