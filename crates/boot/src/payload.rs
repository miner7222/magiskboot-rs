...
/// Helper trait to allow `dyn ReadSeek` trait objects (Rust doesn't allow `dyn Read + Seek`).
...
pub fn extract_boot_from_payload(
    ...
    let mut reader: BufReader<Box<dyn ReadSeek>> = BufReader::new(if in_path == "-" {
        // Read all of stdin into memory so we have a seekable stream
        let mut buf = Vec::new();
