...
impl Cpio {
    ...
    fn extract_entry(&self, path: &str, out: &mut String) -> LoggedResult<()> {
        ...
        match entry.mode & S_IFMT {
            ...
            S_IFREG => {
                let mut file = std::fs::File::create(out.as_str())?;
                file.write_all(&entry.data)?;
            }
            S_IFLNK => {
                // Write symlink target as file content (no real symlinks needed for cpio)
                let target = str::from_utf8(entry.data.as_slice())?;
                let mut file = std::fs::File::create(out.as_str())?;
    ...
    fn add(&mut self, mode: mode_t, path: &str, file: &mut String) -> LoggedResult<()> {
        ...
        let metadata = std::fs::symlink_metadata(file.as_str())?;
...
impl Cpio {
    ...
    fn restore(&mut self) -> LoggedResult<()> {
        ...
        // Collect backup keys first, then remove them (stable alternative to extract_if)
        let backup_keys: Vec<String> = self
            .entries
            .keys()
