...
fn impl_from_args_struct(
    ...
    let trait_impl = quote_spanned! { impl_span =>
        ...
        impl #impl_generics crate::argh::FromArgs for #name #ty_generics #where_clause {
            #from_args_method
        }
...
fn impl_from_args_struct_from_args<'a>(
    ...
    let parse_subcommands = if let Some(subcommand) = subcommand {
        ...
        quote_spanned! { impl_span =>
            Some(crate::argh::ParseStructSubCommand {
                subcommands: <#ty as crate::argh::SubCommands>::COMMANDS,
                dynamic_subcommands: &<#ty as crate::argh::SubCommands>::dynamic_commands(),
                parse_func: &mut |__command, __remaining_args| {
                    #name = Some(<#ty as crate::argh::FromArgs>::from_args(__command, __remaining_args)?);
                    Ok(())
                },
            })
        }
    ...
    let method_impl = quote_spanned! { impl_span =>
        fn from_args(__cmd_name: &[&str], __args: &[&str])
            -> std::result::Result<Self, crate::argh::EarlyExit>
        {
            #![allow(clippy::unwrap_in_result)]

            #( #init_fields )*

            crate::argh::parse_struct_args(
                __cmd_name,
                __args,
                crate::argh::ParseStructOptions {
                    arg_to_slot: &[ #( #flag_str_to_output_table_map ,)* ],
                    slots: &mut [ #( #flag_output_table, )* ],
                    help_triggers: &[ #( #help_triggers ),* ],
                },
                crate::argh::ParseStructPositionals {
                    positionals: &mut [
                        #(
                            crate::argh::ParseStructPositional {
                                name: #positional_field_names,
                                slot: &mut #positional_field_idents as &mut dyn crate::argh::ParseValueSlot,
                            },
                        )*
                    ],
            ...
            let mut #missing_requirements_ident = crate::argh::MissingRequirements::default();
            #(
...
/// Implement `crate::argh::TopLevelCommand` or `crate::argh::SubCommand` as appropriate.
fn top_or_sub_cmd_impl(
    ...
    if type_attrs.is_subcommand.is_none() {
        ...
        quote! {
            #[automatically_derived]
            impl #impl_generics crate::argh::TopLevelCommand for #name #ty_generics #where_clause {}
        }
    } else {
        ...
        quote! {
            #[automatically_derived]
            impl #impl_generics crate::argh::SubCommand for #name #ty_generics #where_clause {
                const COMMAND: &'static crate::argh::CommandInfo = &crate::argh::CommandInfo {
        ...
        match field.kind {
            FieldKind::Option | FieldKind::Positional => {
                let from_str_fn = match &field.attrs.from_str_fn {
                    ...
                    None => {
                        quote! {
                            <#field_type as crate::argh::FromArgValue>::from_arg_value
                        }
                    }
                };

                quote! {
                    let mut #field_name: crate::argh::ParseValueSlotTy<#field_slot_type, #field_type>
                        = crate::argh::ParseValueSlotTy {
            ...
            FieldKind::Switch => {
                quote! { let mut #field_name: #field_slot_type = crate::argh::Flag::default(); }
            }
        }
    })
}
...
/// For each non-optional field, add an entry to the `crate::argh::MissingRequirements`.
                    ...
                    quote! {
                        if #field_name.is_none() {
                            #mri.missing_subcommands(
                                <#ty as crate::argh::SubCommands>::COMMANDS
                                    .iter()
                                    .cloned()
                                    .chain(
                                        <#ty as crate::argh::SubCommands>::dynamic_commands()
                                            .iter()
                                            .copied()
                                    ),
                            )
...
fn impl_from_args_enum(
    ...
    let dynamic_from_args =
        dynamic_type_and_variant
            ...
            .map(|(dynamic_type, dynamic_variant)| {
                quote! {
                    if let Some(result) = <#dynamic_type as crate::argh::DynamicSubCommand>::try_from_args(
                        command_name, args) {
    ...
    let dynamic_commands = dynamic_type_and_variant.as_ref().map(|(dynamic_type, _)| {
        quote! {
            fn dynamic_commands() -> &'static [&'static crate::argh::CommandInfo] {
                <#dynamic_type as crate::argh::DynamicSubCommand>::commands()
            }
        }
    });
                ...
                -> std::result::Result<Self, crate::argh::EarlyExit>
            {
                let subcommand_name = if let Some(subcommand_name) = command_name.last() {
                    *subcommand_name
                    ...
                    return Err(crate::argh::EarlyExit::from("no subcommand name".to_owned()));
                };

                #(
                    if subcommand_name == <#variant_ty as crate::argh::SubCommand>::COMMAND.name {
                        return Ok(#name_repeating::#variant_names(
                            <#variant_ty as crate::argh::FromArgs>::from_args(command_name, args)?
                        ));
                    }
                )*
                ...
                Err(crate::argh::EarlyExit::from("no subcommand matched".to_owned()))
            }
        }

        impl #impl_generics crate::argh::SubCommands for #name #ty_generics #where_clause {
            const COMMANDS: &'static [&'static crate::argh::CommandInfo] = &[#(
                <#variant_ty as crate::argh::SubCommand>::COMMAND,
            )*];
